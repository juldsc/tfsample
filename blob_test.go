...
func TestBlobStorage(t *testing.T) {
	...
	url := "https://myappdata0930.blob.core.windows.net/"
	...
	_, err = client.CreateContainer(ctx, containerName, nil)
        defer cleanupContainer(client, ctx, containerName)
	handleError(err)
...
func cleanupContainer(client *azblob.Client, ctx context.Context, containerName string) {
	_, err := client.DeleteContainer(ctx, containerName, nil)
        handleError(err)
}
